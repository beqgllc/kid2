...
export interface SeoMeta {
  ...
  canonical: string;
  image?: string;
  type?: 'website' | 'article' | 'music.album' | 'music.song' | 'music.playlist' | 'profile' | 'music'
  keywords?: string[];
  jsonLd?: SeoJsonLd;
}
...
export function usePageMeta(meta: SeoMeta) {
  useEffect(() => {
    ...
    const setMeta = (selector: string, attr: 'name' | 'property', value: string, content: string) => {
      let tag = document.querySelector(`${selector}[${attr}="${value}"]`) as HTMLMetaElement | null;
      if (!tag) {
    ...
    setMeta('meta', 'property', 'og:image', meta.image ?? '/images/hero/hero-home.webp');
    setMeta('meta', 'name', 'twitter:card', 'summary_large_image');
    setMeta('meta', 'name', 'twitter:title', meta.title);
    setMeta('meta', 'name', 'twitter:description', meta.description);
    setMeta('meta', 'name', 'twitter:image', meta.image ?? '/images/hero/hero-home.webp');
    ...
    const jsonLdScript = document.querySelector('#attikid-seo-json-ld') as HTMLScriptElement | null;
...
export function buildAlbumJsonLd(album: { title: string; description?: string | null; artist_name: string; release_date: string; slug: string; cover_url?: string | null }) {
  return {
    '@context': 'https://schema.org',
    '@type': 'MusicAlbum',
    name: album.title,
    ...
    image: album.cover_url ?? '/images/hero/hero-home.webp',
